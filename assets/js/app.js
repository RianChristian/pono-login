...
// set audio for alarm
const audio = new Audio('https://assets.mixkit.co/sfx/preview/mixkit-alarm-digital-clock-beep-1.mp3');
audio.loop = true;
...
function ringing(now) {
    audio.play();
    alert(`Hallo, Waktunya Minum Obat! ${now}`)
}
...
function showNewAlarm(newAlarm) {
    ...
    <li class = "time-list">        
        ...
        <button class="deleteAlarm time-control btn btn-primary" id="delete-button" onclick = "remove(this.value)" value=${newAlarm}>Delete Alarm</button>       
    </li>`
...
addAlarm.addEventListener('submit', e => {
    ...
    if (isNaN(newAlarm)) {
        if (!alarmList.includes(newAlarm)) {
            ...
            alert(`Alarm ${newAlarm} Berhasil Diatur.`);
        }
    } else {
        alert("Alarm Gagal diatur!")
    }
})
